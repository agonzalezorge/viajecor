...
export function interpretarFila(numero, celdas) {
  ...
  if (typeof monto !== 'number' || !Number.isFinite(monto)) {
    return problema(`el monto dice "${monto}" y no es un número`);
  }
  if (monto < 0) {
...
export function interpretarPlanilla(filas) {
  ...
  return { movimientos, problemas, comprobaciones: comprobar(sumas, acumulados) };
}
 ...
 */
export function comprobar(sumas, acumulados) {
  ...
  for (const mes of [...sumas.keys()].sort()) {
    ...
    // Un céntimo de diferencia es redondeo, no un dato perdido: la planilla
    // acumula sumando importes ya redondeados a dos decimales, y ese último
    // céntimo puede caer para cualquier lado.
    const diferencia = importado - enLaPlanilla;
    comprobaciones.push({ mes, importado, enLaPlanilla, diferencia, coincide: Math.abs(diferencia) <= 1 });
  }
